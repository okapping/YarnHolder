...
struct YarnsListView: View {
    ...
    @AppStorage("orderDirection") var orderDirection = 0
...
//    Label("KEY_SORT_DESCENDING", systemImage: "arrow.down").tag(1)
    ...
    public func sortedYarns(from yarns: [YarnInfo]) -> [YarnInfo] {
        ...
        let filteredYarns = yarns.filter{ yarn in
            ...
            // 検索時↓
            let name = searchData.name.isEmpty ||
            yarn.name.contains(searchData.name)
            
            let tag = searchData.tags == [] ||
            searchData.tags.allSatisfy { yarn.tags.contains($0) }
//            yarn.tags.contains {searchData.tags.contains($0) }

            return matchesFolder && name && tag
        }
        // 手動（不使用）
...
//        }
        // 作成日潤
        if showYarnsListOrder == 1 && orderDirection == 0 {
            return filteredYarns.sorted(by: { $0.createdAt < $1.createdAt })
        } else if  showYarnsListOrder == 1 && orderDirection == 1 {
            return filteredYarns.sorted(by: { $0.createdAt > $1.createdAt })
        }
        // 毛糸名順
        else if showYarnsListOrder == 2 && orderDirection == 0 {
            return filteredYarns.sorted(by: { $0.name < $1.name })
        } else if showYarnsListOrder == 2 && orderDirection == 1 {
            return filteredYarns.sorted(by: { $0.name > $1.name })
        }
        // 在庫数順
        else if  showYarnsListOrder == 3 && orderDirection == 0 {
            return filteredYarns.sorted(by: { $0.stocksCount < $1.stocksCount })
        } else if  showYarnsListOrder == 3 && orderDirection == 1 {
            return filteredYarns.sorted(by: { $0.stocksCount > $1.stocksCount })
        }
        // 例外処理
        else {
...
//    public func searchedYarns(from yarns: [YarnInfo]) -> [YarnInfo] {
...
//                                Label("手動", systemImage: "gear").tag(0)
                            ...
                            Picker(
                                selection: $orderDirection.animation(),
...
//                            Text("\(String(localized: orderName)) - \(String(localized: sortName))")
                        ...
                        Menu {
                            Picker(
                                ...
                                label: Label("表示設定", systemImage: "list.bullet")
                            ) {
                                Label("シンプル", systemImage: "list.bullet").tag(0)
                                Label("詳細リスト", systemImage: "rectangle.grid.1x2").tag(1)
                            }

                        } label: {
                            Label("表示設定", systemImage: "list.bullet")
                            Text(yarnsListDisplayMode == 0 ? "シンプル" : "詳細リスト")
                        }
                        if let folder = folder {
                            Button {
            ...
            .navigationTitle(folder?.name ?? String(localized: "KEY_ALL_YARNS"))
            // 新規作成シート
